...
import { inquiries, type InquiryRecord, type InquiryStatus } from "@/data/data";
...
const filteredInquiries = computed(() => inquiryStore.filter((record) => {
  ...
  return (
    ...
    && (!query || [record.code, record.agencyName, record.contactName, record.email, record.phone, record.whatsapp]
...
function createEmptyInquiry(): InquiryRecord {
  return {
    id: "", code: "", agencyCode: "", agencyName: "", contactName: "", contactPosition: "",
    email: "", phone: "", whatsapp: "", countryOrRegion: "", sourceChannel: "", originalMessage: "",
    internalRemark: "", owner: "", nextFollowUpAt: "", lostReason: "", status: "new",
...
function saveInquiry(record: InquiryRecord) {
  ...
  if (current) {
    ...
    inquiryStore.unshift({ ...record, id: `inquiry-${Date.now()}`, createdAt: formatDateTime(new Date()) });
  }
