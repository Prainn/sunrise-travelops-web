...
export function useItineraryEditor(options: ItineraryEditorOptions) {
  function createEmptyItinerary(): ItineraryRecord {
    return {
      ...
      vehiclePlans: createDefaultVehiclePlans(),
  ...
  function createItinerary(record: ItineraryRecord): ItineraryRecord | null {
    ...
    const created: ItineraryRecord = {
      ...
      days: plannedDays,
      endDate: addDays(record.startDate, plannedDays - 1),
    };
    syncQuoteOptions(created);
    options.itineraryStore.unshift(created);
    options.inquiry.value.status = transitionInquiry(
      options.inquiry.value.status,
      "itinerary_created",
    );
  ...
  function createDailyPlans(startDate: string, dayCount: number): ItineraryDayRecord[] {
    return Array.from({ length: dayCount }, (_, index) => ({
      ...
      meals: { breakfast: index > 0, lunch: false, dinner: false },
      transport: "",
      description: "",
      items: [],
    }));
  ...
  function ensurePlannedDays(plan: ItineraryRecord, plannedDays: number) {
    ...
    plan.dailyPlans.push(
      ...createDailyPlans(plan.startDate, plannedDays).slice(plan.dailyPlans.length),
    );
  }
    ...
    if (item.type === "restaurant") {
      ...
      if (previous >= 0) {
        ...
        day.items[previous] = {
          ...
          remark: day.items[previous].remark,
        };
        touchSelectedItinerary();
        return true;
      }
  ...
  function updateQuoteSettings(changes: Partial<Omit<ItineraryQuoteSettings, "options">>) {
    ...
    const sharedDelta = sumMoney(
      (["mealOtherCost", "attractionOtherCost"] as const).map((key) =>
        key in changes ? roundMoney((changes[key] ?? 0) - (quote[key] ?? 0)) : 0,
      ),
    );
    if (sharedDelta || changes.paxOtherCosts) {
      for (const option of quote.options)
        for (const price of option.paxPrices) {
          if (price.adultUnitPrice == null) continue;
          const oldPaxCost = quote.paxOtherCosts.find((cost) => cost.pax === price.pax);
          ...
          price.adultUnitPrice = Math.max(
            0,
            sumMoney([price.adultUnitPrice, sharedDelta, paxDelta]),
          );
        }
    }
  ...
  function updateQuoteOption(optionId: string, changes: Partial<Omit<ItineraryQuoteOption, "id">>) {
    ...
    const option = options.selectedItinerary.value?.quote.options.find(
      (record) => record.id === optionId,
    );
    if (!option) return;
    ...
    plan.guidePlans = [
      {
        ...
        referencePrice: guide.dailyPrice,
  ...
  function updateGuidePrice(dailyPrice: number) {
    ...
    if (guide) {
      guide.dailyPrice = normalizeQuoteValue(dailyPrice);
      guide.adjustmentReason = "";
      touchSelectedItinerary();
    }
  }
  function updateHotelCost(tier: ItineraryHotelTier, destination: string, price: number) {
    if (!options.canEditContent()) return;
    const hotel = options.selectedItinerary.value?.hotelPlans
      .find((p) => p.tier === tier)
      ?.hotels.find((h) => h.destination === destination);
    if (hotel) {
      hotel.unitCost = normalizeQuoteValue(price);
      hotel.adjustmentReason = "";
      touchSelectedItinerary();
    }
  }

  function updateHotelPlanSelection(
  ...
  function updateVehiclePlan(tier: ItineraryVehicleTier, value: ItineraryVehiclePlan) {
    ...
    value.pricingMode =
      value.totalPrice === calculateVehiclePlanAutomaticTotal(value) ? "automatic" : "manual";
  ...
  function syncDestinations(plan: ItineraryRecord) {
    ...
    plan.dailyPlans.forEach((day) => {
      if (day.overnightDestination && !destinationSet.has(day.overnightDestination))
  ...
  function syncQuoteOptions(plan: ItineraryRecord) {
    ...
    plan.quote.options = HOTEL_PLAN_TIERS.flatMap((hotelTier) => {
      ...
      return VEHICLE_PLAN_TIERS.flatMap((vehicleTier) => {
        ...
        const option =
          existing.get(key) ??
          createDefaultQuoteOption(hotelTier, vehicleTier, createId("quote-option"));
        option.paxPrices = plan.paxTiers.map((pax) => ({
          pax,
          adultUnitPrice:
            option.paxPrices.find((price) => price.pax === pax)?.adultUnitPrice ?? null,
        }));
        return [option];
      });
    });
  ...
  function cloneQuoteSettings(quote: ItineraryRecord["quote"]): ItineraryRecord["quote"] {
    return {
      ...quote,
      paxOtherCosts: quote.paxOtherCosts.map((cost) => ({ ...cost })),
      staffRoomCosts: quote.staffRoomCosts.map((cost) => ({ ...cost })),
      transportFees: quote.transportFees.map((fee) => ({ ...fee })),
      options: quote.options.map((option) => ({
        ...option,
        paxPrices: option.paxPrices.map((price) => ({ ...price })),
      })),
    };
  }

  function cloneHotelPlans(hotelPlans: ItineraryHotelPlan[]): ItineraryHotelPlan[] {
    return hotelPlans.map((plan) => ({
      ...plan,
  ...
  function cloneVehiclePlans(vehiclePlans: ItineraryVehiclePlan[]) {
    return vehiclePlans.map((plan) => ({
      ...plan,
      arrangements: plan.arrangements.map((a) => ({
        ...a,
        vehicles: a.vehicles.map((v) => ({ ...v })),
      })),
    }));
  }
