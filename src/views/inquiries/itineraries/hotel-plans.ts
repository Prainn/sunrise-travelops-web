...
export function getIncompleteHotelPlanTiers(itinerary: Pick<ItineraryRecord, "destinations" | "hotelPlans">) {
  return getEnabledHotelPlans(itinerary)
    .filter((plan) => !isHotelPlanComplete(plan, itinerary.destinations))
    .map((plan) => plan.tier);
}
