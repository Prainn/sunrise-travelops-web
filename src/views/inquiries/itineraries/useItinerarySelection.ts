...
export function useItinerarySelection() {
  ...
  const rows = computed(() =>
    [...itineraryStore].sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)),
  );
  const selectedItineraryId = ref("");
  const selectedItinerary = computed(() =>
    itineraryStore.find((item) => item.id === selectedItineraryId.value),
  );
  ...
  async function reload() {
    ...
    try {
      const [record, plans] = await Promise.all([
        inquiryService.detail(inquiryId.value),
        inquiryService.listItineraries(inquiryId.value),
      ]);
      ...
      itineraryStore.splice(0, itineraryStore.length, ...plans);
      selectedItineraryId.value = getDefaultItineraryId(
        plans,
        String(route.query.itineraryId ?? selectedItineraryId.value),
      );
    } catch (error) {
      if (version === requestVersion) loadError.value = error instanceof Error ? error.message : "";
    } finally {
  ...
  watch(
    ...
    () => {
      selectedItineraryId.value = getDefaultItineraryId(
        rows.value,
        String(route.query.itineraryId ?? ""),
      );
    },
  );
  return {
