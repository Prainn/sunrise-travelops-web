...
export function getResourcePriceOptions(
  resources: PricingResources,
  guestCount = 0,
): ResourcePriceOption[] {
  return [
    ...resources.hotels
      .filter((hotel) => hotel.status === "enabled")
      .map((hotel) => ({
        id: `hotel:${hotel.id}`,
        type: "hotel" as const,
        resourceId: hotel.id,
        ...
        details: [
          ...
          {
            labelKey: "resource.starRating",
            value: `hotel.ratings.${hotel.rating}`,
          ...
          {
            labelKey: "hotel.individualPrice",
            value: hotel.individualPrice,
    ...
    ...resources.attractions
      .filter((attraction) => attraction.status === "enabled")
      .flatMap((attraction) =>
        attraction.prices.map((price) => ({
          ...
          details: [
            ...
            {
              labelKey: "attraction.category",
              value: attractionCategoryLabelKeys[attraction.category],
              format: "translation" as const,
            },
            ...
            { labelKey: "itinerary.resourceRemark", value: attraction.remark },
            {
              ...
              value: attractionItemTypeLabelKeys[price.itemType],
              format: "translation" as const,
            },
            ...
            {
              labelKey: "attraction.effectivePeriod",
              value:
                price.startDate && price.endDate ? `${price.startDate} — ${price.endDate}` : "",
            },
            { labelKey: "attraction.rackPrice", value: price.rackPrice, format: "money" as const },
            {
              labelKey: "attraction.settlementPrice",
              value: price.settlementPrice,
              format: "money" as const,
            },
            {
              labelKey: "attraction.freeTicket",
              value: price.isFree ? "common.yes" : "common.no",
              format: "translation" as const,
            },
    ...
    ...resources.restaurants
      .filter((restaurant) => restaurant.status === "enabled")
      .flatMap((restaurant) =>
        restaurant.prices.map((price) => ({
...
export function calculateItem(
  ...
  return {
    ...
    unit: option.unit,
    referencePrice: option.unitCost,
    referenceBasis: "resource_price",
    adjustmentReason: "",
    unitCost,
    totalCost: multiplyMoney(
      option.unit === "table" && option.dinerCount
        ? roundMoney(unitCost / option.dinerCount)
        : unitCost,
      quantity,
    ),
    remark: "",
...
export function recalculateItem(item: ItineraryResourceItem) {
  item.totalCost = multiplyMoney(
    item.unit === "table" && item.dinerCount
