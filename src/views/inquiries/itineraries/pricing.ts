...
export const resourcePriceOptions: ResourcePriceOption[] = [
  ...hotels.flatMap((hotel) => hotel.roomTypes.flatMap((roomType) => roomType.pricePlans.map((price) => ({
    id: `hotel:${price.id}`,
    type: "hotel" as const,
    resourceId: hotel.id,
  ...
  ...attractions.flatMap((attraction) => attraction.prices.map((price) => ({
  ...
  ...restaurants.flatMap((restaurant) => restaurant.prices.map((price) => ({
