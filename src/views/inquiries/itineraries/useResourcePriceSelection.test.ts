...
describe("resource price selection requests", () => {
  it("requests nothing while closed, one typed page on open and one detail on selection", async () => {
  ...
  it("uses server search, city filtering and requested page size without loading other pages", async () => {
    ...
    selection.filterOptions("门票"); await flush();
    selection.page.value = 2; selection.pageSize.value = 20; await selection.loadOptions();
    expect(api.getPriceOptions).toHaveBeenCalledTimes(4);
    expect(api.getPriceOptions).toHaveBeenLastCalledWith("restaurant", { page: 2, pageSize: 20, city: "大理", keyword: "门票" });
    expect(api.getPriceSelection).not.toHaveBeenCalled();
  });
  it("ignores stale lists and details after another selection or closing", async () => {
    const { props, selection } = setup(); props.modelValue = true; await flush();
    let oldList!: (value: unknown) => void;
    api.getPriceOptions.mockImplementationOnce(() => new Promise(resolve => { oldList = resolve; }));
    selection.filterOptions("old"); selection.filterOptions("new"); await flush();
    oldList(pageResult("old")); await flush(); expect(selection.visibleOptions.value[0].id).toBe("a");
