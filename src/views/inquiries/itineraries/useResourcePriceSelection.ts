...
import {
  calculateItem,
  recalculateItem,
  getResourcePriceOptions,
  getDefaultResourceQuantity,
  type ResourcePriceOption,
} from "./pricing";

export function useResourcePriceSelection(
  props: {
  ...
  const type = computed<ItineraryDailyItemType>(() =>
    props.mealSlot ? "restaurant" : "attraction",
  );
  const city = ref("");
  ...
  const referencePrice = computed(() =>
    props.currentItem && props.currentItem.resourcePriceId === selectedOption.value?.resourcePriceId
      ? (props.currentItem.referencePrice ?? null)
      : (selectedOption.value?.unitCost ?? null),
  );
  ...
  const baseCanSubmit = computed(() =>
    props.mealSlot && source.value === "custom"
      ? Boolean(customName.value.trim()) &&
        ...
        Number.isFinite(customPrice.value) &&
        customPrice.value >= 0 &&
        Number.isInteger(customQuantity.value) &&
        customQuantity.value > 0
  ...
  const reasonRequired = computed(() => {
    ...
    const sameSource =
      current?.resourceId === option.resourceId &&
      current.resourcePriceId === option.resourcePriceId;
    ...
    async (id) => {
      ...
      try {
        ...
        if (version === detailVersion)
          selectedOption.value = getResourcePriceOptions(resources, props.guestCount)[0];
      } catch {
  ...
  watch(
    ...
    (visible) => {
      ...
      if (visible) {
        ...
        const custom =
          item?.type === "restaurant" && item.resourceId === null && item.resourcePriceId === null;
        ...
        if (item && !custom && item.resourceId && item.resourcePriceId) {
          ...
          selectedOption.value = {
            ...
            resourcePriceId: item.resourcePriceId,
  ...
  function createItem(): ItineraryResourceItem | undefined {
    ...
    if (props.mealSlot && source.value === "custom" && customPrice.value != null) {
      return {
        ...
        resourceName: customName.value.trim(),
        priceName: "",
        unit: customUnit.value,
        unitCost: customPrice.value,
        quantity: customQuantity.value,
        dinerCount: customUnit.value === "table" ? dinerCount.value! : null,
        totalCost: multiplyMoney(
          customUnit.value === "table"
            ? roundMoney(customPrice.value / dinerCount.value!)
            : customPrice.value,
    ...
    if (current && current.resourcePriceId === option.resourcePriceId) {
      const item = {
        ...current,
        unitCost: actualPrice.value,
        adjustmentReason: adjustmentReason.value,
        quantity: quantity.value,
        dinerCount: current.unit === "table" ? dinerCount.value! : null,
      };
      recalculateItem(item);
      return item;
    }
    const item = calculateItem(
      { ...option, dinerCount: dinerCount.value },
      quantity.value,
      actualPrice.value,
    );
