...
export function getEnabledVehiclePlans(itinerary: Pick<ItineraryRecord, "vehiclePlans">) {
  return itinerary.vehiclePlans.filter((plan) => plan.arrangements.length);
}
...
export function calculateVehiclePlanAutomaticTotal(plan: ItineraryVehiclePlan) {
  if (
    !plan.arrangements.length ||
    plan.arrangements.some((arrangement) => arrangement.totalPrice == null)
  )
    return null;
...
export function isVehiclePlanTotalOverridden(plan: ItineraryVehiclePlan) {
  ...
  return (
    ...
    roundMoney(plan.totalPrice) !== automaticTotal
  );
}
export function withVehiclePlanArrangements(
...
export function getIncompleteVehiclePlanTiers(
  itinerary: Pick<ItineraryRecord, "paxTiers" | "vehiclePlans">,
) {
  ...
  return getEnabledVehiclePlans(itinerary)
    .filter((plan) => {
      ...
      if (
        sorted.some(
          (arrangement, index) => index > 0 && arrangement.startDate <= sorted[index - 1].endDate,
        )
      )
        return true;
