...
export function changeTone(
  change: Pick<Change, "before" | "after">,
...
export function formatLogValue(value: unknown, path: string, context: DisplayContext): string {
  ...
  if (Array.isArray(value))
    return value.map((item) => formatLogValue(item, path, context)).join("\n");
  ...
  if (
    ["nextFollowUpAt", "createdAt", "updatedAt", "quoteGeneratedAt", "occurredAt"].includes(
      field ?? "",
    )
  )
  ...
  switch (field) {
    case "mealSource":
      key = value === "custom" ? "itinerary.customRestaurant" : "itinerary.mealLibrary";
      break;
    case "status":
      key = `${targetType}.statuses.${value}`;
      break;
    case "tier":
    case "hotelTier":
    case "vehicleTier":
      key = `inquiry.log.tiers.${value}`;
      break;
    case "secondLanguage":
      key = `planning.languages.${value}`;
      break;
    case "rating":
      key = `hotel.ratings.${value}`;
      break;
    case "mealSlot":
      key = `itinerary.meals.${value}`;
      break;
    case "cabin":
      key = `itinerary.cabins.${value}`;
      break;
    case "type":
      key = path.includes("transportFees")
        ? `itinerary.feeCards.${value}`
        : `itinerary.resourceTypes.${value}`;
      break;
  }
