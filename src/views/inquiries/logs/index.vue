<template>
  <div v-loading="isLoading" class="page-container inquiry-log-page">
    <el-card class="log-summary-card shrink-0" shadow="never">
      <el-page-header @back="router.push({ name: 'InquiryList' })">
        <template #content>
          {{ inquiry ? `${inquiry.code} · ${inquiry.agencyName}` : $t("inquiry.log.report") }}
        </template>
      </el-page-header>
      <el-form :inline="true" class="log-filters mt-[20px]">
        <el-form-item
          v-if="userStore.userInfo.scope === 'headquarters' && !inquiryId"
          :label="$t('identity.businessUnit')"
        >
          <el-select v-model="businessUnit" class="!w-[180px]" @change="search">
    ...
    <el-card class="page-content" shadow="never">
      ...
      <div class="page-table-wrapper">
        ...
        >
            ...
            <template #default="{ row }">
              <div class="change-details p-[16px]">
                ...
                <el-table
                  ...
                  :row-class-name="
                    ({ row: change }) =>
                      `change-${changeTone(change as InquiryLogRecord['changes'][number])}`
                  "
                  :show-overflow-tooltip="false"
                  border
                >
                  <el-table-column :label="$t('inquiry.log.field')" min-width="240">
                    <template #default="scope">
                      {{ fieldLabel(scope.row as InquiryLogRecord["changes"][number]) }}
                    </template>
                  </el-table-column>
                  <el-table-column :label="$t('inquiry.log.before')" min-width="280">
                    <template #default="scope">
                      <pre
          ...
          <el-table-column :label="$t('inquiry.log.time')" min-width="175">
            <template #default="{ row }">
          ...
          />
          <el-table-column :label="$t('inquiry.log.operator')" min-width="150">
            <template #default="{ row }">
...
const actions: InquiryLogAction[] = [
  "inquiry_created",
  "inquiry_updated",
  "itinerary_created",
  "itinerary_saved",
...
const action = ref<InquiryLogAction | "">("");
const page = ref(1);
...
async function fetchLogs() {
  ...
  const query = {
    businessUnit:
      userStore.userInfo.scope === "headquarters" && !inquiryId.value
        ? businessUnit.value || undefined
        : undefined,
    page: page.value,
    pageSize: pageSize.value,
    inquiryCode: inquiryCode.value,
    inquiryId: inquiryId.value || undefined,
  ...
  try {
    const [details, summary, people, record] = await Promise.all([
      inquiryLogService.list(query),
      inquiryLogService.report(query),
      inquiryLogService.operators(),
      inquiryId.value ? inquiryService.detail(inquiryId.value) : Promise.resolve(undefined),
    ...
    const itineraryIds = [
      ...new Set(
        details.list
          .filter(
            (row) =>
              row.targetType === "itinerary" &&
              row.changes.some((change) => change.path.endsWith(".dayIds")),
          )
          .map((row) => row.targetId),
      ),
    ];
    const itineraryRecords = await Promise.all(
      itineraryIds.map((id) => inquiryService.itinerary(id)),
    );
    if (version !== requestVersion) return;
    logs.value = details.list;
    total.value = details.total;
    report.value = summary;
    operators.value = people;
    inquiry.value = record;
    itineraryDays.value = Object.fromEntries(
      itineraryRecords.map((record) => [
        record.id,
    ...
    if (version !== requestVersion) return;
    error.value = reason instanceof Error ? reason.message : t("request.failed");
    logs.value = [];
    total.value = 0;
    report.value = {
      totalOperations: 0,
...
function search() {
  if (page.value === 1) void fetchLogs();
  else page.value = 1;
}
watch(
  [inquiryId, page, pageSize],
  () => {
    void fetchLogs();
  },
  { immediate: true },
);
function formatAction(row: InquiryLogRecord) {
  return t(`inquiry.log.actions.${row.action}`);
}
function fieldName(name: string) {
  const key = `inquiry.log.fields.${name}`;
  return te(key) ? t(key) : name;
}
function fieldLabel(change: InquiryLogRecord["changes"][number]) {
  const context = change.context;
  const prefix = [
    context?.dayNumber ? t("inquiry.log.day", { day: context.dayNumber }) : "",
    context?.name,
    context?.destination,
    context?.hotelTier ? t(`inquiry.log.tiers.${context.hotelTier}`) : "",
    context?.vehicleTier ? t(`inquiry.log.tiers.${context.vehicleTier}`) : "",
  ]
    .filter(Boolean)
    .join(" · ");
...
<style scoped>
.inquiry-log-page :deep(.el-table__expand-icon.is-disabled) {
  visibility: hidden;
}

.change-details :deep(.change-added) {
  color: var(--el-color-success);
}
.change-details :deep(.change-removed) {
  color: var(--el-color-danger);
}
.change-details :deep(.change-changed) {
