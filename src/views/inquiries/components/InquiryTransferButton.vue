<template>
  ...
  <el-dialog v-model="visible" :title="$t('identity.transfer')" width="480px" append-to-body>
    <el-form ref="form" :model="data" label-position="top">
      ...
      >
        <el-select v-model="data.ownerId" filterable :placeholder="$t('common.selectPlaceholder')">
          <el-option
...
const visible = ref(false),
  saving = ref(false),
  owners = ref<PersonOption[]>([]),
  form = ref<FormInstance>();
const data = reactive({ ownerId: "", reason: "" });
async function open() {
  try {
    owners.value = await inquiryService.owners(props.inquiry.businessUnit);
    data.ownerId = "";
    data.reason = "";
    visible.value = true;
  } catch (e) {
...
async function submit() {
  ...
  saving.value = true;
  try {
    await inquiryService.transfer(props.inquiry, data.ownerId, data.reason);
    visible.value = false;
    emit("transferred");
  } catch (e) {
    ElMessage.error(e instanceof Error ? e.message : String(e));
