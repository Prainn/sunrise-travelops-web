  ...
  <div class="page-container">
    <el-form inline class="page-search">
      ...
      <el-form-item class="w-300px">
        <el-input v-model="keyword" :placeholder="$t('guide.searchPeople')" clearable />
      </el-form-item>
    ...
    <el-card class="page-content" shadow="never">
      ...
      <div class="page-table-wrapper">
        <el-table v-loading="loading" :data="rows" border height="100%" row-key="id">
          ...
          <el-table-column prop="name" :label="$t('guide.name')" min-width="160" />
          <el-table-column :label="$t('guide.gender')" min-width="100">
            <template #default="{ row }">
              {{ $t(`guide.genderOptions.${row.gender}`) }}
            </template>
          </el-table-column>
          <el-table-column :label="$t('guide.age')" min-width="90">
            <template #default="{ row }">
          ...
          <el-table-column :label="$t('guide.employmentType')" width="140">
            <template #default="{ row }">
              {{
                row.employmentType
                  ? $t(`guide.${row.employmentType === "full_time" ? "fullTime" : "partTime"}`)
                  : $t("common.notSet")
              }}
            </template>
          </el-table-column>
            ...
            <template #default="{ row }">
              {{
                row.hasLaborContract === null
                  ? $t("common.notSet")
                  : $t(row.hasLaborContract ? "guide.contractYes" : "guide.contractNo")
              }}
            </template>
          </el-table-column>
          <el-table-column
          ...
          <el-table-column :label="$t('guide.certificateNo')" min-width="220">
            <template #default="{ row }">
              {{ row.certificateNo ?? $t("common.notSet") }}
            </template>
          </el-table-column>
          <el-table-column :label="$t('guide.identityNumber')" min-width="220">
            <template #default="{ row }">
          ...
          <el-table-column :label="$t('common.actions')" min-width="260" fixed="right">
            <template #default="{ row }">
              <el-button link type="primary" @click="emit('detail', row as GuidePersonRecord)">
                {{ $t("guide.detail") }}
              </el-button>
              <el-button
