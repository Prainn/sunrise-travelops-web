...
type BusinessDictionaryTypeForm = Pick<
  BusinessCategoryTypeRecord,
...
function syncBuiltInStores(types: BusinessCategoryTypeRecord[]): void {
  ...
  if (resourceUnitType) {
    resourceUnitStore.splice(
      0,
      resourceUnitStore.length,
      ...resourceUnitType.items.map(
        (item) =>
          ({
            ...item,
  ...
  if (transportMethodType) {
    transportMethodStore.splice(
      0,
      transportMethodStore.length,
      ...transportMethodType.items.map(
        (item) =>
          ({
            ...item,
            code: item.code,
...
export const businessDictionaryService = {
  ...
  async updateType(
    id: string,
    data: BusinessDictionaryTypeForm,
  ): Promise<BusinessCategoryTypeRecord> {
  ...
  async getItems(
    typeCode: string,
    query: BusinessDictionaryItemQuery = {},
  ): Promise<BusinessCategoryOptionRecord[]> {
  ...
  async createItem(
    typeCode: string,
    data: BusinessDictionaryItemForm,
  ): Promise<BusinessCategoryOptionRecord> {
