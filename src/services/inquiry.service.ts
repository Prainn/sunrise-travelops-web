...
const MONEY_FIELDS = new Set(["hotelUnitCost","vehicleTotal","guideServiceTotal","staffRoomTotal","vehicleUnitCost","guideServiceUnitCost","staffRoomUnitCost","leaderUnitPrice","tipUnitPrice","profitPerPerson","referencePrice","segmentTotal","beforePrice","afterPrice","unitCost","totalCost","total","dailyPrice","adultUnitPrice","unitPrice","chineseTip","englishTip","childUnitPrice","hotelCost","vehicleCost","commonGroupCost","baseGroupCost","baseCostPerPerson","singleSupplementUnitCost","totalPrice","profit","dailyResourceCost","mealOtherCost","attractionOtherCost","mealCost","attractionCost","guideCost","otherExpenses"]);
...
export function itineraryInput(record: Pick<ItineraryRecord, "title" | "startDate" | "paxTiers" | "childRate" | "destinations" | "dailyPlans" | "hotelPlans" | "vehiclePlans" | "guidePlans" | "quote">) {
  return { title: record.title, startDate: record.startDate, paxTiers: record.paxTiers, childRate: record.childRate, destinations: record.destinations, dailyPlans: record.dailyPlans, hotelPlans: record.hotelPlans, vehiclePlans: record.vehiclePlans, guidePlans: record.guidePlans, quote: record.quote };
}
