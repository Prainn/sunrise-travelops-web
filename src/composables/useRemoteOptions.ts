...
export interface RemoteOptionsQuery {
  keyword: string;
  page: number;
  pageSize: number;
}
...
export function useRemoteOptions<T extends { id: string }>(
  ...
  const hasMore = computed(
    () => !failed.value && page.value > 0 && items.value.length < total.value,
  );
  let version = 0;
  ...
  async function loadMore() {
    ...
    try {
      ...
      items.value = [
        ...new Map([...items.value, ...result.list].map((item) => [item.id, item])).values(),
      ];
  ...
  async function search(value: string) {
    ...
    if (normalized === keyword.value && (loading.value || (page.value > 0 && !failed.value)))
      return;
    reset(normalized);
    await loadMore();
  }
